...
#   cat("--> WRITE\n")
#   js <- write_bds(tgt, file = file.path(path, paste0("test", i, ".json")), format = outformat, indent = 2)
#   cat("--- NEXT\n")
#   rm(tgt)
# }
#
...
# js <- write_bds(tgt, file = file.path(path, "http400.json"), format = outformat, indent = 2)
#
# tgt <- read_bds(fns[2], format = "1.0", append_ddi = TRUE, verbose = TRUE)
# js <- write_bds(tgt, file = file.path(path, "not_a_vector.json"), format = outformat, indent = 2)
